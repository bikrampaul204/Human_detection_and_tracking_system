...
names=["","","Bikram","Aditi","Shubham"]#the names of the people that could be tracked
    ...
    while True:
        if (flag==0):#during the begin of tracking person unrecognized so flag=0
            ...
            try:
                ...
                cv2.imshow("Human", frame)#display the tracking box
                ...
                if ok:
                    ...
                    for (x, y, w, h) in faceDetect:#go through the faces that were detected
                        if x>=(bbox[0]-30) and y<=(bbox[1]+30) and w<=(bbox[2]+30) and h<=(bbox[3]+30):
                            ...
                            if round(confidence,2)<=55 and round(confidence,2)>0:#if confidence is greater than 60, then check among the Id's
                                flag=Id                                
...
#this function is used for background subtraction, if there is no sufficient
#difference between the two images then they are considered as the same image
...
if __name__ == '__main__':
    ...
    while True:
        ...
        min_area=(3000/800)*frame_resized.shape[1]# defining min cutoff area

        while True:
          previous_frame = frame_resized_grayscale# store the previous captured frame
          grabbed, frame = camera.read()#capture a frame
          cv2.imshow("frame", frame)#display the tracking box
          k = cv2.waitKey(1) & 0xff
          if k == 27 : break
          if not grabbed:
            break
